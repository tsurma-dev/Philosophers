...
/*   Updated: 2024/06/17 16:05:06 by tsurma           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	allocs(pthread_t **s, t_philo **t, pthread_mutex_t **f, t_house *h)
{
	...
	*f = malloc(((h->nmb_philo * 3) + 3) * sizeof(pthread_mutex_t));
	if (!*f)
	{
		free(*s);
		free(*t);
...
static int	assignments(t_philo *tablet, pthread_mutex_t *forks, t_house *house)
{
	...
	while (++i <= house->nmb_philo)
	{
		...
		tablet[i].mute_thread = &forks[house->nmb_philo * 3 + 2];
...
int	thread_weaver(t_house *house)
{
	...
	i = 0;
	house->start = gtod();
	pthread_mutex_lock(&forks[house->nmb_philo * 3 + 2]);
	while (++i <= house->nmb_philo)
		pthread_create(&symposium[i], NULL, &socrates, &tablet[i]);
	pthread_mutex_unlock(&forks[house->nmb_philo * 3 + 2]);
	i = 0;
	while (++i <= house->nmb_philo)
